"""
FIX FILE 4 — src/agents/fact_checker.py  (full replacement)
Fixes:
  P1-1  No retry on requests.get — now uses tenacity
  P1-6  Source-index mismatch when pro_sources has empty inner lists — guard added
  P2-1  Failed futures produce no error records — now appends ERROR SourceVerification
  P2-2  confidence=1.0 hardcoded — now set to actual avg verification rate
  P3-2  import random inside thread body — moved to module level
"""
import logging
import random                              # P3-2 fix: module-level import
import re
import threading
...
class FactChecker(BaseAgent):
    ...
    def generate(self, state: DebateState) -> AgentResponse:
        ...
        # P1-6 fix: guard against length mismatches between sources and arguments
            ...
            try:
                for future in concurrent.futures.as_completed(futures, timeout=60):
                    ...
                    try:
                        ...
                        # P2-1 fix: append an error record so rates are accurate
                        results.append(SourceVerification(
                            url=url, status="TIMEOUT", agent_source=None,
                            error="Verification timed out"
                        ))
                    except Exception as e:
                        logger.error("Failed to verify %s: %s", url, e)
                        # P2-1 fix: append an error record
                        results.append(SourceVerification(
                            url=url, status="ERROR", agent_source=None, error=str(e)
                        ))
            except concurrent.futures.TimeoutError:
                logger.warning("FactChecker batch verification timed out after 60s")
                # P2-1 fix: any pending futures get error records
        ...
        # P2-2 fix: confidence reflects actual verification rate, not hardcoded 1.0
        total        = len(results)
        total_verified = pro_verified + con_verified
        overall_confidence = (total_verified / total) if total > 0 else 0.0
        ...
        return AgentResponse(
            ...
            argument=(
                ...
                f"PRO: {pro_verified}/{len(pro_results)} verified. "
                f"CON: {con_verified}/{len(con_results)} verified."
            ),
            sources=[r.url for r in results],
            confidence=round(overall_confidence, 3),   # P2-2 fix
            metrics={
    ...
    # P1-1 fix: retry decorator on _verify_url's inner fetch logic
    @retry(
    ...
    def _fetch_url(self, url: str) -> requests.Response:
        """Isolated fetch so retry only covers network errors, not logic errors."""
        headers = {
    ...
    def _verify_url(self, url: str, agent: str, claim: str = "",
                    claim_embedding: Any = None,
                    state: Optional[DebateState] = None) -> SourceVerification:
        """Deterministic URL verification."""
        try:
            if not isinstance(url, str):
                return SourceVerification(url=str(url), status="INVALID_URL",
                                          agent_source=agent, error="Non-string URL")

            url = url.strip()
            parsed = urlparse(url)
            if not (parsed.scheme in ("http", "https") and parsed.netloc):
                return SourceVerification(url=url, status="INVALID_URL",
                                          agent_source=agent,
                                          error="Invalid URL format: no scheme or host")

            # Content-first: reuse pre-fetched Tavily snippet if available
            if state:
                for pool in [state.evidence_sources, state.pro_evidence, state.con_evidence]:
                    ...
                    for item in pool:
                        if item.get("url") == url and item.get("content"):
                            logger.debug("Using pre-fetched snippet for %s", url)
                            return self._process_content(url, item["content"],
                                                         agent, claim, claim_embedding)

            resp = self._fetch_url(url)   # P1-1 fix: retried fetch
            ...
            paywall_indicators = [
                ...
                "login to continue", "subscription required", "members only",
            ]
            if any(ind in content_text.lower() for ind in paywall_indicators):
                return SourceVerification(url=url, status="PAYWALL_RESTRICTED",
                                          agent_source=agent,
                                          error="Content behind paywall or login")

            if resp.status_code == 200:
                return self._process_content(url, content_text, agent, claim, claim_embedding)

            return SourceVerification(url=url, status="NOT_FOUND",
                                      agent_source=agent,
                                      error=f"HTTP {resp.status_code}")

        except requests.Timeout:
            return SourceVerification(url=str(url), status="TIMEOUT",
                                      agent_source=agent, error="Request timeout")
        except requests.RequestException as e:
            return SourceVerification(url=str(url), status="ERROR",
                                      agent_source=agent, error=str(e))
        except Exception as e:
    ...
    def _process_content(self, url: str, content: str, agent: str,
                         claim: str = "",
                         claim_embedding: Any = None) -> SourceVerification:
        ...
        if claim and re.search(r"\b(?:19|20)\d{2}\b", claim):
            ...
            if not is_aligned:
                return SourceVerification(
                    url=url, status="CONTENT_MISMATCH", agent_source=agent,
        ...
        if self.model and claim:
            ...
            if candidates:
                ...
                sentence_embs    = self.model.encode(candidates)
                ...
                if max_sim > threshold:
                    return SourceVerification(
                        ...
                        content_preview=candidates[int(np.argmax(similarities))][:200],
                    )
                return SourceVerification(
                    url=url, status="CONTENT_MISMATCH", agent_source=agent,
                    error=f"Semantic similarity too low: {max_sim:.2f}",
